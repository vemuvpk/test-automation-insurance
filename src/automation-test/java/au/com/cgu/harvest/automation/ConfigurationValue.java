 ...
 * <p/>
 * Note that system property values that contain a $ in the result are ignored. This is to work
 * around an issue with Maven, where non-existent environment values are passed in as literals.
 * <p/>
 * You can also pass in the text string "null" as an override mechanism (because there is no "unset"
 * option to completely null out an already set value in Maven)
 */
public final class ConfigurationValue
{
    ...
    public static String getConfigurationValue( String key, String defaultValue )
    {
        ...
        if ( System.getProperties().containsKey( key ) )
        {
            ...
            log.trace( String.format( "found System Property key [%s] set to [%s]", key, result ) );
        }
        else if ( System.getenv().containsKey( key ) )
        {
            result = getValueIfSet( System.getenv( key ) );
            log.trace( String.format( "found Environment Property key [%s] set to [%s]", key,
                result ) );
        }
        else
        {
            result = defaultValue;
            log.trace( String.format( "Using default key [%s] set to [%s]", key, result ) );
        }
